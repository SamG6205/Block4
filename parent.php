<?php 
// Start session to store users session info.
session_start();
...
<html>
    <head>
        <title>Rishton Academy Parents</title>
    ...
    <body>
      ...
      <!-- Made referencing https://www.w3schools.com/bootstrap5/bootstrap_navbar.php -->
        ...
        <main class="container-fluid" style="text-align: center;">
            <p class="display-2"> Parents</p>
          <hr>
            <button class="collapseBtn btn btn-secondary mb-2" data-bs-toggle="collapse" data-bs-target="#addParent">Add Parent</button><br>
            <div id="addParent" class="collapse show">
              <?php
              ...
              if ($_POST['submit'] == 'Submit') {
                // Check all required fields and not empty
                if (strlen($_POST['fname']) >0 and strlen($_POST['lname'])>0) {
                  if (strlen($_POST['mobile']) >0 and strlen($_POST['telephone'])>0) {
                    if (strlen($_POST['addressID'])>0) {
                      ...
                      try {
                        if (mysqli_query($link, $sql)) {
                          ...
                          $email = $_POST['email']??'';
                          $sql = "INSERT INTO Parents (personID ,email, telephone, mobile) VALUES ('$person' ,'$email','{$_POST['telephone']}','{$_POST['mobile']}')";
                              if (mysqli_query($link, $sql)){
              ...
              ?>
                <form  action="parent.php" method="post">
                    ...
                    <div class="input-group mb-2">
                        ...
                        <input id="mobile" name="mobile" class="form-control" placeholder="Mobile Phone number" value="<?php echo "{$_POST['mobile']}" ?>" type='number' required>
                        <label for="telephone" class="input-group-text" type='number'> Telephone</label>
                        <input id="telephone" name="telephone" class="form-control" placeholder="Telephone number" value="<?php echo "{$_POST['telephone']}" ?>" required>
                    </div>
                  <div>
                    <?php
  // Stage system to check which stage of finding address the user is at, and echos the relavant html code.
  if (!isset($_POST['stage']) or ($_POST['stage'] == 0)) {
    echo "<div class='input-group mb-2'>
                        ...
                        ";
                                 // Select address matching the given postcode.
                        ...
                        ";
                               ...
                               elseif ($_POST['stage'] == '2') {
                                 if ($_POST['address'] > 0 ) {
                                   // Select choosen address form database
          ...
          <hr>
            <button class="collapseBtn btn btn-secondary" data-bs-toggle="collapse" data-bs-target="#viewParents">View Parents</button><br>
            <div id="viewParents" class="collapse">
            <br>
                <table class="center">
                    <tr>
                    ...
                    <th width="250px">Mobile<br><hr></th>
                    <th width="300px">Email address<br><hr></th>
                ...
                // Select parents from database and show in the table.    
                $sql = mysqli_query($link, "
                SELECT Parents.personID,Persons.firstname, Persons.lastname, Parents.mobile,
                Parents.email 
                FROM Parents 
